...
- [ ] 實作「基準對象」下拉選單（從 DB 動態讀取可選項目）
- [ ] 實作「對比對象」下拉選單（同上）
- [ ] 支援選項格式：Site A / 2025 Q1

B-2 動態對比引擎

- [ ] 實作 Site A vs. Site B（同季度跨廠比較）
- [ ] 實作 YoY 成長分析（同廠不同年度比較）
- [ ] 實作 AvB 預算執行率分析（Actual vs. Budget，需資料包含 Budget 類別）

B-3 核心指標圖表（KPI Cards & Charts）

- [ ] 營收與毛利：顯示增減額（Amount）與增減率（%）
- [ ] 費用率分析：計算並比較兩者的 OpEx %（opex / revenue）
- [ ] 費用率超標預警：設定閾值，超標時顯示警示標記
- [ ] 以圖表（Bar / Line Chart）呈現對比結果

B-4 靜態報告匯出

- [ ] 實作「匯出 Excel」功能，輸出當前對比數據表
- [ ] 實作「匯出 PDF」功能，輸出格式化分析報告
